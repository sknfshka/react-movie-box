...
export function addComment(userId, filmId, rating, title, text) {
  const filmIndex = filmsWithDetails.findIndex(film => film.id === filmId);
  const toAdd = {
    author: {
      name: "Arthur Morgan",
      id: userId,
  ...
  if (filmsWithDetails[filmIndex].comments) {
    filmsWithDetails[filmIndex].comments = [
      ...filmsWithDetails[filmIndex].comments,
      toAdd
    ]
  } else {
  ...
  addCommentToUser(userId, filmId, rating, title, text, toAdd.date, filmsWithDetails[filmIndex].title);
}
